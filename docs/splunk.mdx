...
SPLUNK_VERIFY_SSL=true                     # Set false for self-signed certs (optional)
```
...
| `SPLUNK_VERIFY_SSL` | `true` | Set to `false` when using self-signed TLS certificates |
...
| Symptom | Fix |
| --- | --- |
| `SSL: CERTIFICATE_VERIFY_FAILED` | Set `SPLUNK_VERIFY_SSL=false` (self-signed cert) |
...
- Set `SPLUNK_VERIFY_SSL=false` only in local or dev environments with self-signed certificates.
